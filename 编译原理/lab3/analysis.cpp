...
void Analysis::match(const TokenType& expectedToken) {
    ...
    if (token.second == expectedToken) {
        ...
        Error(2, ErrorLine[tkNow].second, expectedToken,token.second); //报错行数主要指向缺失的下一个字符所在行。
...
void Analysis::Error(int errorKind, const int& idx, const TokenType& expectedToken = ERROR, const TokenType& token = ERROR) {
    QString description;
    QString file = fileName;  // `fileName` 是当前文件名，这里有bug，为空。应该是被清除掉了？

    switch (errorKind) {
    case 1:
        description = QString("某函数内未识别的Token: %1").arg(QString::fromStdString(Tokens[tkNow - 1].first));
        break;
    case 2:
        description = QString("期待的Token类型: %1,   当前匹配到Token类型: %2")
    ...
    // 存储错误信息到 vector
    if(idx < ErrorLine.size())
        errorList.emplace_back(description, file, ErrorLine[idx].second);
    else
        errorList.emplace_back(description, file, ErrorLine[ErrorLine.size()-1].second); //防止报错行数越界，最后一行就指定在最后一行
}
