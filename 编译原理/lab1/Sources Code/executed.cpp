...
* 不支持字符串换行(通过r#原生字符串或者\换行，如需添加，可在原生字符串&&字符串判断部分添加一个类似于多行注释的expFlag标识符标注是否换行。
*
*/
...
int interFace(int index, const string& rust, int rcaFlag = 0) {
    ...
    //判断字符字面量
        ...
        deal += rust[index++];
        if (index + 1 < rust.length() && rust[index] != '\\')
        {

            deal += rust[index++];
            deal += rust[index++];
        }
        else if(index + 2 <rust.length() && rust[index] == '\\')
        {
            deal += rust[index++];
            deal += rust[index++];
            deal += rust[index++];
