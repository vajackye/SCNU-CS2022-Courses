...
//主分析函数
void LR1::analysis(QPlainTextEdit *inputText)
{

    QStringList lines = inputText->toPlainText().split('\n', QString::SkipEmptyParts); // 按行分割文本内容
    ...
    for(const QString &line: lines) {
        ...
        string ss = clearSpaceLine.toStdString(); //将QString转为std::string
        if (ss.empty()) continue; //如果是空行，跳过
    ...
    for (const auto& state : LR0DfaStates) { //遍历每一个状态（项目集）
            ...
            if(item.dotPos == item.right.size()) {//归约项
                ...
                for (const auto& symbol : follow) {
                    ...
                    if (finalFolSet.count(symbol)) { // 非空符号的冲突检查
                        string reason ="在状态: "+ to_string(stateId) + " 存在规约-规约冲突," + " 冲突的规约符号为: " + symbol + "。状态请见左侧LR0_DFA图表";
                        isSlr = false;
        ...
        for (const auto& symbol : shiftFirstSet) {
            if (symbol != "@" && finalFolSet.count(symbol)) { //有交集
                string reason ="在状态: "+ to_string(stateId) + " 存在移进-规约冲突," + " 冲突的移进符号为: " + symbol + "。状态请见左侧LR0_DFA图表";
                isSlr = false;
                return reason; //返回冲突原因
            }
        }
